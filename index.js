...
let names, score = 0, maxscore, tries, nameLayerDict = {};
...
const gameOver = () => {
    document.getElementById('map-side-panel').innerText = `Game over! Your score is ${score}/${maxscore} (${Math.round((100 * score)/maxscore)}%).`;
}
...
const playRegion = (the_region) => {
    ...
    getPlayData(play_path).then((borders) => {
        ...
        let geoj = L.geoJSON(borders, {onEachFeature: (feature, layer) => {
            layer.on({click: handleClick});
            nameLayerDict[feature.properties["name:en"] || feature.properties["name"]] = layer;
        }}).addTo(map);
